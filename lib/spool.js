...
async function createSpoolItem(cfg, connection) {
  ...
  return { id: item.id, path: item.pendingPath, meta: item.meta }
}

async function createTempSpoolItem(cfg, connection) {
  ensureSpoolDirs(cfg)
...
async function commitSpoolItem(item) {
  await fsp.rename(item.path, item.pendingPath)
}

async function discardSpoolItem(item) {
